...
//helper function to join a new game with the given move index and move on behalf of the user address
const joinGame = (moveIndex: number, move: number, user: string) => {
  return simnet.callPublicFn(
    "ttt",
    "join-game",
...
const play = (moveIndex: number, move: number, user: string) => {
  return simnet.callPublicFn(
    "ttt",
    "play",
    [Cl.uint(0), Cl.uint(moveIndex), Cl.uint(move)],
...
describe("TTT Tests", () => {
  ...
  it("allows player one to win", () => {
    ...
    const gameData = simnet.getMapEntry("ttt", "games", Cl.uint(0));
